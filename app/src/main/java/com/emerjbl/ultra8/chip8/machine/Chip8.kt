...
class Chip8(
    ...
    private object Keys {
        ...
        private val keyWaiter = AtomicReference<Continuation<Int>?>(null)
        private val awaitingUp = AtomicInteger(-1)

        fun keyDown(idx: Int) {
            keys.getAndUpdate { it or mask(idx) }
            keyWaiter.get()?.resume(idx)
        }

        fun keyUp(idx: Int) {
            keys.getAndUpdate { it and mask(idx).inv() }
            if (idx == awaitingUp.get()) {
                keyWaiter.getAndSet(null)?.resume(idx)
            }
        }
        ...
        suspend fun awaitKey(): Int {
            ...
            val downNow = keys.get()
            awaitingUp.set(-1)
            val down = if (downNow > 0) {
                downNow.countTrailingZeroBits()
            } else {
                suspendCancellableCoroutine<Int> { keyWaiter.set(it) }
            }
            awaitingUp.set(down)
            suspendCancellableCoroutine<Int> { keyWaiter.set(it) }
            return down
